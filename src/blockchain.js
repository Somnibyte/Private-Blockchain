...
class Blockchain {
    ...
    async submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let secondsElapsed= currentTime - messageTime;

            if(secondsElapsed <= 300) {
                let messageIsValid = bitcoinMessage.verify(message, address, signature);
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(targetBlock != undefined) {
                ...
                reject("No block with hash " + hash + " was found.");
            }
        });
    }
     ...
     async getStarsByWalletAddress (address) {
        ...
        return new Promise( async (resolve, reject) => {
            ...
            if(self.chain.length == 1) {
                reject("Cannot get data from genesis block.");
            }

            for (let i = 1; i < self.chain.length; i++) {
                let blockJsonObj = await self.chain[i].getBData();
                if(blockJsonObj.address == address) {
                    stars.push({"owner": blockJsonObj.address, "star": blockJsonObj.star});
                }
            }
    ...
    async validateChain() {
        let self = this;
        let errorLog = [];
        let errorMessage = "Invalid block was found.";
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i < self.chain.length; i++) {
                ...
                if(i == 0) {
                    if(blockIsValid) {
                        ...
                        errorLog.push({"message": errorMessage, "block": currentBlock})
                    }
                } else {
                    let previousBlock = self.chain[i-1];
                    let blockIsValid = await currentBlock.validate();
                    if(blockIsValid && currentBlock.previousBlockHash == previousBlock.hash) {
                        continue;
                    } else {
                        errorLog.push({"message": errorMessage, "block": currentBlock})
                    }
